...
3. 配置运行环境：
   ...
   - 在 **Deployment** 里添加 Artifact 选择 `javaweb:war exploded`（或 `javaweb.war`），设置应用路径为 `/javaweb`；
   - 在 **Server** 标签页将 **URL** 设为 `http://localhost:8080/javaweb/`。
4. 启动 Tomcat 后访问 <http://localhost:8080/javaweb/> 进入登陆页，体验登陆、会话检查、单点登录，以及商城/购物车/下单等示例流程。
